...
function formatTableColumns(rawTsv) {
  ...
  return rows.map(row => 
    row.map((cell, colIdx) => cell.padEnd(colWidths[colIdx] + 2)).join('')
  ).join('\n');
}
...
export const printFuzzMatchesPivotTable = async function (jq, jsonString) {
  ...
  const uniqueAFilter = '[.[] | select(.depth == 0) | .ai] | unique | map(tostring)[]';
...
export const printFuzzyMatchesDetail = async function (jq, jsonString) {
  ...
  // process.stdout.write("\n\n\n");
  process.stdout.write("FUZZY MATCH Records\n\n");
