...
import { colorize, colorizeToCallback } from './colorize.js';
...
export class JsonDiff {
    ...
    for (const [key, { item: candidate, index: matchIndex }] of Object.entries(fuzzyOriginals)) {
      if (key !== '__next') {
        ...
        if (extendedTypeOf(item) === extendedTypeOf(candidate)) {
          const change = this.diff(candidate, item); // switched the order for consistency (candidate is the original value) and to allow caching (reuse change values calculated here in later fuzzy matching)
          if (
            /* prettier-ignore */
            // change.equal && indexDistance === 0 ||
            !bestMatch ||
  ...
  scalarize(array, originals, fuzzyOriginals) {
    ...
    const fuzzyMatches = [];
    const keyScores = {}; // moving keyScores up to allow change bestMatch.change to be cached for later use in arrayDiff()
    if (fuzzyOriginals) {
  ...
  arrayDiff(obj1, obj2) {
    ...
    if (this.options.sort) { // ??? Seems like this is defeating the purpose of the fuzzy matching. Should be sorting obj1 and obj2 before scalarizing objects
      seq1.sort();
      seq2.sort();
    }
        ...
        case 'equal':
          ...
          for (
            ...
            if (this.isScalarized(item, originals1)) {
              ...
              // const __change = this.diff(item1, item2) // result was already computed, don't repeat
              const change = originals2[item].change
              // console.log("__change === change: ", JSON.stringify(__change) === JSON.stringify(change))
              if (!change.equal) {
