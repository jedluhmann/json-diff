...
import { extendedTypeOf, keysEqual, escapeRegExp, printFuzzMatchesPivotTable, printFuzzyMatchesDetail } from './util.js';
...
export class JsonDiff {
  ...
  set options(opts) {
    this.#options = { outputKeys: [], excludeKeys: [], ...opts };
  }
      ...
      switch (type1) {
        ...
        case 'array':
          if (this.options.precision) {
            obj1 = obj1.map((el) => (Number.isFinite(el) && !Number.isInteger(el) ? Number(el.toFixed(this.options.precision)) : el));
            obj2 = obj2.map((el) => (Number.isFinite(el) && !Number.isInteger(el) ? Number(el.toFixed(this.options.precision)) : el));
          }
  ...
  async printDebug(result) {
    const jq = await loadJq(); // async init once

    const uniquePathstoFirstArray = [...new Set(this.fuzzyMatchRecs.map((r) => (r.path.match(/^[^[]*\[\d+\]$/) ? r.path.split('[')[0] : null)).filter(Boolean))];

    for (const path of uniquePathstoFirstArray) {
      const safePath = escapeRegExp(path);
      const pathRegex = new RegExp(`^${safePath}\\[\\d+\\]$`);
      const filteredFuzzyMatchRecs = this.fuzzyMatchRecs.filter((r) => r.path.match(pathRegex));
    ...
    if (this.options.verbose) {
      const jsonString = JSON.stringify(this.fuzzyMatchRecs);
  ...
  async exec(obj1, obj2) {
    const diff = this.diff(obj1, obj2);
    const result = diff.result;

    if (this.options.debug) {
      await this.printDebug(result);
    }

    if (this.options.color == null) {
      this.options.color = tty.isatty(process.stdout.fd);
    }

    if (result && !this.options.silent) {
      if (this.options.raw) {
        if (this.options.verbose) {
        ...
        const colorizeOptions = {
          ...
          maxElisions: this.options.maxElisions,
        };
