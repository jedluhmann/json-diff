...
import { extendedTypeOf, roundObj, keysEqual, printFuzzMatchesPivotTable, printFuzzyMatchesDetail } from './util.js';
...
export class JsonDiff {
      ...
      if (key !== '__next') {
        ...
        if (extendedTypeOf(item) === extendedTypeOf(candidate)) {
          ...
          if (
            ...
            if (this.options.debug) change.debug = debug;
  ...
  diff(obj1, obj2, path = '$') {
    ...
    if (type1 === type2) {
      switch (type1) {
        case 'object':
          return this.objectDiff(obj1, obj2, path);

        case 'array':
          return this.arrayDiff(obj1, obj2, path);
      }
    }
...
export const diffWithDebug = async function (obj1, obj2, options = {}) {
  ...
  const result = jDiff.diff(obj1, obj2).result;

  if (options.debug) {
    const jq = await loadJq(); // async init once
    const jsonString = JSON.stringify(jDiff.fuzzyMatchRecs);

    await printFuzzMatchesPivotTable(jq, jsonString);
    if (options.verbose) await printFuzzyMatchesDetail(jq, jsonString);
  }
